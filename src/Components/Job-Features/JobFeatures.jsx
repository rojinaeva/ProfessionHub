...
const JobFeatures = (props) => {
    const {id,logo,job_position,company_name,job_type,job_site,job_location,salary_range}=props.job;
    ...
    return (
        <div className='p-4 border border-white rounded-xl'>
           ...
           <div className='mt-4'>
              ...
              <p className='mt-2'><span className='mr-2'><FontAwesomeIcon className='text-white w-6 h-6' icon={faDollarSign} /></span><span className=' text-gray-300 font-semibold font-serif text-xl'>Salary : {salary_range}</span></p>
           </div>
        <Link to={`/job/${id}`}> <button onClick={()=>handleViewDetails(props.job)} className='bg-zinc-400 px-3 py-3 rounded-xl font-extrabold text-white text-xl hover:bg-zinc-500 mt-6'>View Details</button></Link>
